...
void input(Scheduler& scheduler,std::vector<ProcessControlBlock>& processes) {
    ...
    double processPriority = 0,processArrivalTime = 0,processBurstTime = 0,processUsedCupTime = 0,processState;
    int dividedId = 1;
    
    while(std::cin >> processName) {
        std::cin >> processArrivalTime >> processPriority >> processBurstTime;
...
int main() {
    ...
    startFirstComeFirstServe(scheduler,processes);
